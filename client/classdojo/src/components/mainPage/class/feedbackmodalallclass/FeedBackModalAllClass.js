...
export default function FeedBackModalAllClass({ open, handleClose, classroom }) {
    ...
    useEffect(() => {
        if (document.getElementById(`skill-df-open-${classroom._id}`)) {
            document.getElementById(`skill-df-open-${classroom._id}`).click();
        }
    }, [open, classroom])
    const plusSkillAllStudent = async (skill) => {
        try {
            setLoading(true);
            disPatch({ type: 'IS_PLUS' })
            await axios.post(`/class/plus_whole_class/${classroom._id}`, {
                name: skill.name,
                number: skill.number,
                avatar: skill.avatar
            }, {
            ...
            disPatch(classAction.plusSkillAllStudent(classroom._id, skill));

            disPatch({ type: 'NO_PLUS' })
            ShowToastifyPlussSkillAllStudent("👨‍🏫 Phản hồi cho cả lớp thành công")
            handleClose()
                ...
                <Fade in={open}>
                    <div className={classes.paper}>
                        <div className={classes.heading}>
                            <h2>Đưa ra phản hồi cho {classroom.students.length} học sinh</h2>
                            <i className='bx bx-x' onClick={handleClose}></i>
                        </div>
                        <div className='skills'>
                            <div className="skills__tab-list">
                                <ul className={`skills__tab-list-${classroom._id}`}>
                                    <li onClick={(e) => openTab(e, `pos-${classroom._id}`)} id={`skill-df-open-${classroom._id}`}>
                                        Tích cực
                                    </li>
                                    <li onClick={(e) => openTab(e, `neg-${classroom._id}`)}>
                                        Tiêu cực
                                    </li>
                                </ul>
                            </div>
                            <div className={`skills__content skills__content-${classroom._id}`} id={`pos-${classroom._id}`}>
                                <div className='skills__content-list'>
                                    {loading !== true ? classroom.skills && classroom.skills.map((skill, index) => {
                                        if (parseInt(skill.number) > 0) {
                                            return <div onClick={() => plusSkillAllStudent(skill)} key={index} className='skills__content-list-item feedback__skill'>
                            ...
                            <div className={`skills__content skills__content-${classroom._id}`} id={`neg-${classroom._id}`}>
                                <div className='skills__content-list'>
                                    {loading !== true ? classroom.skills && classroom.skills.map((skill, index) => {
                                        if (parseInt(skill.number) < 0) {
                                            return <div onClick={() => plusSkillAllStudent(skill)} key={index} className='skills__content-list-item feedback__skill'>
